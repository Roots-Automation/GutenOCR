...
def generate_fake_bounding_boxes(
    image_width: int, image_height: int, granularity: str = "word", num_boxes: int = None
) -> list[list[int]]:
...
def get_available_text_types(example: dict[str, Any]) -> list[str]:
    ...
    if (
        "lines" in available_types
        or "words" in available_types
        or "latex" in available_types
        or "paragraphs" in available_types
    ):
...
def extract_answer(
    ...
    input_type: str,
    task_type: str,
    sampled_item: dict[str, Any] | None = None,
) -> Any:
    ...
    if grounded_output is not None:
        ...
        grounded_output = sorted(
            grounded_output, key=lambda x: (x.get("bbox", [0, 0, 0, 0])[1], x.get("bbox", [0, 0, 0, 0])[0])
        )
        ...
        if task_type == "conditional_detection":
            ...
            if input_type_clean.startswith("[") and input_type_clean.endswith("]"):
                ...
                if granularity and text_data.get(granularity):
                    boxes = [normalize_keys(item)["bbox"] for item in text_data[granularity]]
            ...
            available_granularities = [
                name for name in ["words", "lines", "latex", "paragraphs"] if text_data.get(name)
            ]
            ...
            boxes = [normalize_keys(item)["bbox"] for item in text_data[chosen_granularity]]
            return sorted(boxes, key=lambda x: (x[1], x[0]))  # sort by y1, then x1
    ...
    if is_localized:
        ...
        if output_type in ["text", "text2d"]:
            ...
            return sorted(
                paragraphs_data, key=lambda x: (x.get("bbox", [0, 0, 0, 0])[1], x.get("bbox", [0, 0, 0, 0])[0])
            )
...
def format_question(
    question: str, example: dict[str, Any], input_type: str, task_type: str = None, output_type: str = None
...
def generate_empty_text_prompt(
    example: dict[str, Any], allowed_tasks: list[str] | None = None, allowed_output_types: list[str] | None = None
) -> tuple[str, Any]:
    ...
    if selected_task_type == "reading":
        ...
        for task in tasks:
            if task["task"] == "reading" and task["input_type"].strip() == "image":
                ...
                if (
                    (output_type == "[words, box]" and "words" in available_text_keys)
                    or (output_type == "[lines, box]" and "lines" in available_text_keys)
                    or (output_type == "[paragraphs, box]" and "paragraphs" in available_text_keys)
                    or (output_type in available_text_keys)
                ):
        ...
        if allowed_output_types is not None:
            ...
            reading_tasks = [
                task for task in reading_tasks if task["output_type"].strip().lower() in allowed_output_types_lower
            ]
        ...
        if allowed_output_types is not None:
            allowed_output_types_lower = [ot.lower() for ot in allowed_output_types]
            localized_tasks = [
                task for task in localized_tasks if task["output_type"].strip().lower() in allowed_output_types_lower
            ]
...
def generate_prompt(
    example: dict[str, Any],
    tasks_csv_path: str | None = None,
    seed: int | None = None,
    allowed_tasks: list[str] | None = None,
    allowed_input_types: list[str] | None = None,
    ...
    if allowed_tasks is not None:
        ...
        if not compatible_tasks:
            available_categories = list(
                set(task["task"] for task in tasks if check_task_compatibility(task, available_types))
            )
            raise ValueError(
                f"No compatible tasks found for allowed task types: {allowed_tasks}. Available: {available_categories}"
            )
    ...
    if allowed_input_types is not None:
        ...
        compatible_tasks = [
            task for task in compatible_tasks if task["input_type"].strip().lower() in allowed_input_types_lower
        ]

        if not compatible_tasks:
            available_input_types = list(set(task["input_type"] for task in tasks))
            raise ValueError(
                f"No compatible tasks found for allowed input types: {allowed_input_types}. Available: {available_input_types}"
            )
    ...
    if allowed_output_types is not None:
        allowed_output_types_lower = [ot.lower() for ot in allowed_output_types]
        compatible_tasks = [
            task for task in compatible_tasks if task["output_type"].strip().lower() in allowed_output_types_lower
        ]

        if not compatible_tasks:
            available_output_types = list(set(task["output_type"] for task in tasks))
            raise ValueError(
                f"No compatible tasks found for allowed output types: {allowed_output_types}. Available: {available_output_types}"
            )
    ...
    formatted_question, sampled_item = format_question(
        selected_task["question"],
        example,
        selected_task["input_type"],
        selected_task["task"],
        selected_task["output_type"],
    )
    expected_answer = extract_answer(
        example, selected_task["output_type"], selected_task["input_type"], selected_task["task"], sampled_item
    )
...
def main():
    ...
    example = {
        "text": {
            "lines": [{"text": "Hello World", "box": [80, 60, 280, 90]}],
            "latex": [{"text": "\\omega = \\Pi_{i=1} x_i^\\tau", "box": [80, 160, 140, 190]}],
            "text": "Hello World",
            "text2d": "    Hello\n  World",
        },
        "image": {"path": "example.jpg", "width": 800, "height": 600},
    }
