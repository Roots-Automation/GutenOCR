...
A vLLM-optimized evaluation harness for the [Fox benchmark](https://github.com/ucaslcl/Fox), enabling efficient multi-GPU inference for fine-grained document OCR tasks.
...
# Summary metrics: ROUGE-L
python3 eval_tools/eval_summary_test.py --out_file "./results_final.json"

# QA metrics: Exact-match accuracy
python3 eval_tools/eval_qa_test.py --out_file "./results_final.json"
```

### OpenAI-Compatible Local Server

-   You can run models via an OpenAI-style local server (e.g., vLLM `serve`). Set env vars:


    export VLLM_OPENAI_BASE_URL="http://localhost:8000/v1"
    export VLLM_OPENAI_API_KEY="EMPTY"

-   Then run the provided script for PaddleOCR-VL:


    bash scripts/paddleocr_vl_openai.sh

-   This uses the OpenAI backend in `fox_eval.py`, sending images as base64 `image_url` entries.

## Contact

For questions about this vLLM evaluation harness, please open a GitHub issue.

For questions about the original Fox benchmark, contact the authors via the [Fox repository](https://github.com/ucaslcl/Fox).
