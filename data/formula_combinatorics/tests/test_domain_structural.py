...
def test_algebra_power_frequency() -> None:
    n = _freq(_ALGEBRA, "^{")
    assert n >= 700, f"'^{{' appeared only {n}/2000 times in algebra"
...
def test_trigonometry_sin_frequency() -> None:
    n = _freq(_TRIG, r"\sin")
    assert n >= 1000, f"\\sin appeared only {n}/2000 times in trigonometry"


def test_trigonometry_cos_frequency() -> None:
    n = _freq(_TRIG, r"\cos")
    assert n >= 200, f"\\cos appeared only {n}/2000 times in trigonometry"
...
def test_linear_algebra_ampersand_frequency() -> None:
    n = _freq(_LA, "&")
    assert n >= 1200, f"'&' appeared only {n}/2000 times in linear_algebra"
